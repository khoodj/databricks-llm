...
class SQLCustomTools(SQL_LLM, LangchainDB):
    ...
    def create_sql_command(question: str) -> str:
        ...
        sql_rev1 = query_object_chain.invoke(
            {"question": question}  #  + "The SQL query generated MUST BE correct.",
        ).sql_query
    ...
    @staticmethod
    @tool
    def execute_sql_command(sql_query: str) -> Union[pd.DataFrame, str]:
        ...
        Returns:
            Union[pd.DataFrame, str]: The result of the SQL query or an error message.
        """
        ...
        return df if not df.empty else error_message
    ...
    @staticmethod
    @tool
    def fix_sql_query(error_msg: str, sql_query: str) -> str:
        """Fixes an SQL query that has an error. This tool is meant to be used after the execute_sql_command tool returns an error message.

        Args:
            error_msg (str): The error message from the execute_sql_command tool
        ...
        return query_object_chain.invoke(
            {
                "question": f"Fix the SQL query: {sql_query} that is causing the error message: {error_msg}"
            }
        ).sql_query
